/** ATLAS TIME — مقارنة المدن: حفظ محلي وتصدير SVG آمن بصيغتي PNG وPDF. */
...
export default function Compare() {
  ...
  const buildExportSvg = (theme: ExportTheme) => {
    ...
    const dateTime = new Intl.DateTimeFormat(isAr ? "ar-SA" : "en-GB", { year: "numeric", month: "long", day: "numeric", hour: "2-digit", minute: "2-digit", timeZone: selectedCity.timezone, timeZoneName: "short" }).format(exportedAt);
    ...
    const exported = isAr ? `تاريخ ووقت التصدير: ${dateTime}` : `Exported: ${dateTime}`;
    ...
    const columnLabels = `<text x="1460" y="234" text-anchor="end" class="label">${xmlSafe(labels[0])}</text><text x="970" y="234" text-anchor="middle" class="label">${xmlSafe(labels[1])}</text><text x="670" y="234" text-anchor="middle" class="label">${xmlSafe(labels[2])}</text><text x="320" y="234" text-anchor="middle" class="label">${xmlSafe(labels[3])}</text>`;
    const body = rows.map((city, index) => {
  ...
  const downloadPdf = async () => {
    try {
      ...
      pdf.save(`atlas-time-comparison-${exportTheme}-${new Date().toISOString().slice(0, 10)}.pdf`);
      setExportState("pdf");
