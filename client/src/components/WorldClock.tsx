      ...
      <div className="watch-stage">
        ...
        <motion.div className="watch-case" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.65, ease: [0.23, 1, 0.32, 1] }}>
          ...
          <div className="dial-center">
            ...
            <div className="main-time" dir="ltr" aria-live="polite"><AnimatePresence mode="popLayout"><motion.span key={`${time.hour}${time.minute}`} initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -9, opacity: 0 }} transition={{ duration: 0.25 }}>{time.hour}:{time.minute}</motion.span></AnimatePresence><span className="seconds-readout"><i aria-hidden="true">:</i><AnimatePresence mode="popLayout" initial={false}><motion.em key={`tens-${time.second[0]}`} initial={{ y: 5, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -5, opacity: 0 }} transition={{ duration: 0.17, ease: [0.23, 1, 0.32, 1] }}>{time.second[0]}</motion.em></AnimatePresence><AnimatePresence mode="popLayout" initial={false}><motion.em key={`units-${time.second[1]}`} initial={{ y: 5, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -5, opacity: 0 }} transition={{ duration: 0.17, ease: [0.23, 1, 0.32, 1] }}>{time.second[1]}</motion.em></AnimatePresence></span></div>
