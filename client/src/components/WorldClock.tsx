      ...
      <div className="watch-stage">
        ...
        <motion.div className="watch-case" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.65, ease: [0.23, 1, 0.32, 1] }}>
          ...
          <div className="dial-center">
            ...
            <div className="main-time" aria-live="polite"><AnimatePresence mode="popLayout"><motion.span key={`${time.hour}${time.minute}`} initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -9, opacity: 0 }} transition={{ duration: 0.25 }}>{time.hour}:{time.minute}</motion.span></AnimatePresence><em>{time.second}</em></div>
